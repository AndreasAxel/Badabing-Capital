...
if __name__ == '__main__':
    ...
    # Variables to vary
    alpha = [25]          # Regularization (MSE of price only, MSE of price and delta)
    deg_poly = [3, 5, 7, 9]     # Degree of Polynomials
    N_train = np.array([128 * 2 ** i for i in range(8)])
    ...
    def hedge_expirement(alpha, deg):
        ...
        for k, n in enumerate(N_train):
            ...
            a[0, :] = np.minimum(delta_ub, np.maximum(
                delta_lb, letourneau_fit_predict(lsmc=lsmc, x=S[0, :], x0=K, deg_stentoft=deg)[1]
            ))
            b[0, :] = binom[0] - a[0, :] * S[0, :]
            V[0, :] = b[0, :] + a[0, :] * S[0, :]
            ...
            a[0, :] = \
                ...
                diff_reg_fit_predict(x=np.array([x0]), t=t, N_train=n, r=r, sigma=sigma, K=K,
            ...
            for j, s in enumerate(t[1:], start=1):
                ...
                a[j, :] = letourneau_fit_predict(lsmc=lsmc, x=S[j, :], x0=K, deg_stentoft=deg)[1]
    ...
    color_alpha = {0.5: 'orange', 25: 'blue'}
    ...
    #plt.text(np.min(N_train), df['STD_HEDGE_ERR'].max() * 1.15, 'Low dispersion (α = 0.5)', color='blue')
    #plt.text(np.min(N_train), df['STD_HEDGE_ERR'].max() * 1.10, 'High dispersion (α = 25)', color='blue')
    plt.show()
