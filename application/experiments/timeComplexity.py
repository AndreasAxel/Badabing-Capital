...
if __name__ == '__main__':
    ...
    computation_time = np.full((len(N_train), 3), np.nan)
    ...
    for k, n in tqdm(enumerate(N_train)):
        ...
        t_diff_reg = np.full_like(S, np.nan)

        a_diff_reg[0, :] = np.minimum(delta_ub, np.maximum(
            delta_lb, diff_reg_fit_predict(x=np.array([x0]), t=t, N_train=n, r=r, sigma=sigma, K=K, deg=deg_diffreg, option_type=option_type)[1]
        ))
        ...
        # Update results
        computation_time[k, 0] = np.mean(t_ls[1:,:])
        computation_time[k, 1] = np.mean(t_diff_reg[1:,:])
        computation_time[k, 2] = np.mean(t_nn[1:,:])
    ...
    for i in range(3):
        plt.plot(N_train,(computation_time[:, i]), color=color[i], label=label[i], marker='o')
    plt.semilogx(base=2)
    plt.semilogy(base=2)
    plt.xticks(ticks=N_train, labels=N_train)
    plt.legend()
    plt.ylim(-0.1, np.max(computation_time) * 1.2)
    plt.ylabel('Avg. Computation Time (sec)')
